    ...
    <div className="overflow-x-hidden bg-sl-bg font-sans antialiased">
      <div className="sl-container sl-page-top pb-section-end">
        <div className="sl-display">
          <span className="sl-display-title">About</span>
        </div>
        {/* Stats */}
        {stats.length > 0 && (
          <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4">
            {stats.map((s, i) => (
              <div key={s.id} className={`sl-reveal sl-d${(i % 4) + 1}`}>
                <div className="text-[clamp(36px,5vw,48px)] leading-none font-bold tracking-[-0.035em] text-sl-text">
                  {s.value}
                </div>
        ...
        {values.length > 0 && (
          <section className="sl-container sl-section-sm">
            <h2 className="sl-reveal sl-section-heading">What we value</h2>
